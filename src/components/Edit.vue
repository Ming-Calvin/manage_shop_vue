  ...
  methods: {
    ...
    async next () {
      await this.$refs.addFormRef.validate(async valid => {
        ...
        this.active++
        const { data: res } = await this.$http.put('commoditys/' + this.editId, this.addForm)
        console.log(res)
    ...
    gainEdit () {
      ...
      console.log(this.addForm, '2')
      if (this.$route.query.status === 'true') {
        this.addForm.status = true
      } else {
        this.addForm.status = false
      }
      if (this.$route.query.sell === 'true') {
        this.addForm.sell = true
      } else {
        this.addForm.sell = false
      }
